...
def test(new_adj, gcn=None, model_name=None):
    ...
    if gcn is None:
        ...
        gcn.fit(attr, new_adj, labels, train_indices, train_iters=400, verbose=True, idx_val=test_indices) # train with validation model picking
        gcn.eval()
...
if __name__ == '__main__':
      ...
      if args.model == 'GCNJaccard':
            ...
            model = model_name(attr.shape[1], 32, 2, device=device,binary_feature=False)
      if args.model == 'GCN':
            model_name = GCN
      ...
      model.fit(features=attr, adj=adj, labels=labels, idx_train=train_indices, idx_val=test_indices, train_iters=400, verbose=True) # val
      ...
      if args.attack == 'Metattack':
          atk_name = Metattack
          atk_model = atk_name(model=model, nnodes=adj.shape[0], device=device, undirected=False)
