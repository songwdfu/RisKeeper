...
if __name__ == '__main__':
      ...
      # node_costs = pkl.load(open(f'/data/wenda/GCN_ADV_Train/bitcoin_data/{args.dataset}_{args.perturb_ratio}_node_cost.pkl', 'rb'))
      node_costs = np.zeros([adj.shape[0], 1])
      
      total_edges = adj.sum()/2
      eps = total_edges * args.perturb_ratio
      # Setup Attack Model
      print('=== setup attack model ===')
      if args.attack == 'Metattack':
          atk_name = Metattack
          atk_model = atk_name(model=model, nnodes=adj.shape[0], device=device, undirected=False, lambda_=0.5, node_costs=node_costs, hyper_c=0.001*253.45/eps)
