...
class PGDAttack(BaseAttack):
    ...
    def attack_cost(self, ori_features, ori_adj, labels, idx_train, cost_constraint, node_costs, epochs=200, hyper_c=1, **kwargs):
        ...
        edge_costs = np.clip(edge_costs, 1e-5, None)
        upper_triangle_indices = np.triu_indices(edge_costs.shape[0], k=1)
        edge_costs = torch.from_numpy(edge_costs[upper_triangle_indices]).to(self.device)
        edge_costs[edge_costs < 1e-5] = 0
        edge_costs = edge_costs.to(torch.float32)
        ...
        print(f'average cost per edge: {edge_costs.mean()}')
        ...
        for t in tqdm(range(epochs)):
            ...
            cost_loss = hyper_c * torch.matmul(torch.abs(self.adj_changes), edge_costs) * 0.001
    ...
    def projection_cost(self, cost_constraint, edge_costs):
        ...
        if torch.matmul(edge_costs, torch.clamp(self.adj_changes, 0, 1)) > cost_constraint:
            c = edge_costs[edge_costs > 1e-5]
            left = (self.adj_changes-1).min() / c.max()
            cmin = c.min() if c.min()>1e-5 else 1e-5
            right = self.adj_changes.max() / cmin
            miu = self.bisection_cost(left, right, cost_constraint, edge_costs, epsilon=1e-5)
