...
class PGDAttack(BaseAttack):
    ...
    def attack_cost(self, ori_features, ori_adj, labels, idx_train, cost_constraint, total_defense_cost, epochs=200, hyper_c=1, **kwargs):
        ...
        # edge_costs = torch.from_numpy(node_costs + node_costs.T).to(self.device).to(torch.float32)
        ...
        for t in tqdm(range(epochs)):
            ...
            cost_loss = hyper_c * 0.001 * torch.matmul(torch.abs(self.adj_changes), edge_costs)
