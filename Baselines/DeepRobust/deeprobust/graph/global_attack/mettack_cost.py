...
class Metattack(BaseMeta):
    ...
    def __init__(self, model, nnodes, feature_shape=None, attack_structure=True, attack_features=False, undirected=True, device='cpu', with_bias=False, lambda_=0.5, train_iters=100, lr=0.1, momentum=0.9, node_costs=None, hyper_c=None):
        ...
        if node_costs is not None:
            self.edge_costs = torch.tensor(node_costs + node_costs.T).to(device)
    ...
    def inner_train(self, features, adj_norm, idx_train, idx_unlabeled, labels):
        ...
        for j in range(self.train_iters):
            ...
            loss_labeled = F.cross_entropy(output[idx_train], labels[idx_train])
            # loss_labeled += attack_cost
    ...
    def get_meta_grad(self, features, adj_norm, idx_train, idx_unlabeled, labels, labels_self_training):
        ...
        # loss_labeled = F.cross_entropy(output[idx_train], labels[idx_train])
        # loss_unlabeled = F.cross_entropy(output[idx_unlabeled], labels_self_training[idx_unlabeled])
        # loss_test_val = F.cross_entropy(output[idx_unlabeled], labels[idx_unlabeled])
        ...
        ###################
        # # whether consider attack_costs
        cost_loss = self.hyper_c * 0.001 * (torch.abs(self.adj_changes) * self.edge_costs).sum() # should be self.modified_adj
        # attack_loss_final = attack_loss
        attack_loss_final = attack_loss - cost_loss
        #####################
        
        print(f'GCN loss on unlabled data: {loss_test_val.item()}, ...acc: {utils.accuracy(output[idx_unlabeled], labels[idx_unlabeled]).item()}')
        print(f'total loss: {attack_loss_final.item()}, costs loss: {cost_loss.item()}, margin: {attack_loss.item()}')
        

        adj_grad, feature_grad = None, None
        if self.attack_structure:
            adj_grad = torch.autograd.grad(attack_loss_final, self.adj_changes, retain_graph=True)[0]
        if self.attack_features:
            feature_grad = torch.autograd.grad(attack_loss_final, self.feature_changes, retain_graph=True)[0]
        return adj_grad, feature_grad

    def attack(self, ori_features, ori_adj, labels, idx_train, idx_unlabeled, cost_constraint, node_costs, hyper_c, ll_constraint=True, ll_cutoff=0.004):
        ...
        """
        if node_costs is not None:
            self.edge_costs = torch.tensor(node_costs + node_costs.T).to(self.device)
        if hyper_c is not None:
            self.hyper_c = hyper_c
        ...
        while current_cost <= cost_constraint:
            epoch += 1
            print(f'--- epoch: {epoch} ---')
            if self.attack_structure:
            ...
            # cost_loss = self.hyper_c * 0.001 * (torch.abs(self.adj_changes.data) * self.edge_costs).sum() # should be self.modified_adj
            # cost_loss.requires_grads = True
            ...
            if adj_meta_score.max() >= feature_meta_score.max():
                adj_backup = self.adj_changes.data
            ...
            current_cost = (torch.abs(self.adj_changes.data) * self.edge_costs).sum()
            print(f'current sum costs: {current_cost.item()} / constraint: {cost_constraint}')

        self.adj_changes.data = adj_backup # dont exceed constraint
...
#     def __init__(self, model, nnodes, feature_shape=None, attack_structure=True, attack_features=False, undirected=True, device='cpu', with_bias=False, lambda_=0.5, train_iters=100, lr=0.01, hyper_c=0.01):
...
#     def inner_train(self, features, modified_adj, idx_train, idx_unlabeled, labels, labels_self_training, edge_costs):
#         adj_norm = utils.normalize_adj_tensor(modified_adj)
#         for j in range(self.train_iters):
#             # hidden = features
#             # for w, b in zip(self.weights, self.biases):
...
#             attack_loss += self.hyper_c * (modified_adj * edge_costs).sum()             # attack costs
...
#         loss_test_val = F.nll_loss(output[idx_unlabeled], labels[idx_unlabeled])
#         loss_test_val += self.hyper_c * (modified_adj * edge_costs).sum()               # attack costs
...
#     def attack(self, ori_features, ori_adj, labels, idx_train, idx_unlabeled, n_perturbations, ll_constraint=True, ll_cutoff=0.004, node_costs=None):
#         """Generate n_perturbations on the input graph.
...
#         modified_adj = ori_adj
#         modified_features = ori_features
#         edge_costs = node_costs + node_costs.T
...
#             self.inner_train(modified_features, modified_adj, idx_train, idx_unlabeled, labels, labels_self_training, edge_costs)
