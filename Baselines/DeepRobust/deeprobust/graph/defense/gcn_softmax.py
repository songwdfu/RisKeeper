...
class GCN_softmax(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.01, weight_decay=5e-4,
        ...
        super(GCN_softmax, self).__init__()
    ...
    def fit(self, features, adj, labels, idx_train, idx_val=None, train_iters=200, initialize=True, verbose=False, normalize=True, patience=500, **kwargs):
        ...
        if idx_val is None:
            ...
            if patience < train_iters:
                self._train_with_early_stopping(labels, idx_train, idx_val, train_iters, patience, verbose) # modified
            else:
    ...
    def _train_with_early_stopping_modified(self, labels, idx_train, idx_val, train_iters, patience, verbose):
        if verbose:
            print('=== training gcn model ===')
        optimizer = optim.Adam(self.parameters(), lr=self.lr, weight_decay=self.weight_decay)
        ...
        for i in range(train_iters):
            ...
            # perf_sum = eval_class(output[idx_val], labels[idx_val])
            loss_val = F.nll_loss(output[idx_val], labels[idx_val])
            cost_val.append(loss_val.detach().cpu().numpy())
            if cost_val[-1] < np.mean(cost_val[-11:-1]):
                best_loss_val = loss_val
                self.output = output
                weights = deepcopy(self.state_dict())
            ##if best_loss_val > loss_val:
