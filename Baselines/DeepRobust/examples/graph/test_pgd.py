...
from deeprobust.graph.defense import GCN, GCNJaccard, GCNSVD
...
data = Dataset(root='/data/wenda/BitcoinBaselines/DeepRobust/examples/graph/tmp', name=args.dataset, setting='gcn')
...
if args.surrogate == 'GCN':
    surrogate_name = GCN
if args.surrogate == 'GCNJaccard':
    surrogate_name = GCNJaccard
if args.surrogate == 'GCNSVD':
    surrogate_name = GCNSVD
...
def test(new_adj, gcn=None):
    ...
    if gcn is None:
        ...
        gcn = surrogate_name(nfeat=features.shape[1],
                  nhid=32,
...
def main():
    target_gcn = surrogate_name(nfeat=features.shape[1],
              nhid=32,
              nclass=labels.max().item() + 1,
