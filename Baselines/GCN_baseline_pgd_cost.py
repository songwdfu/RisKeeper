...
def test_median(adj, features, gcn=None, model_name=None):
    ...
    if gcn is None:
        ...
        gcn.fit([pyg_data], train_iters=400, verbose=True, patience=50)
        gcn.eval()
    ...
    loss_test = F.nll_loss(output[test_indices].detach().cpu(), torch.tensor(labels[test_indices], dtype=torch.long))
    acc_test = accuracy(output[test_indices].detach(), torch.tensor(labels[test_indices], dtype=torch.long))
    if nclass == 2:
...
def bisection_beta(x, total_costs):
    ...
    dim = x.shape[0]
    left = 0
    right = -np.log(total_costs/dim) / np.clip(x.min(), 1e-8, None) + 100
    beta = (left + right) / 2 
...
if __name__ == '__main__':
      ...
      parser.add_argument('--dataset', type=str, default='cora', help='Can be "cora", "citeseer", "photo", "computers"')
      ...
      parser.add_argument('--cost_constraint', type=float, default=0.2)
      parser.add_argument('--hyper_c_ratio', type=float, default=1.0)
      ...
      try:
          result = pkl.load(open(f'results/result_{args.model}_{args.attack}_{args.dataset}_{args.cost_scheme}_{args.perturb_ratio}_{args.cost_constraint}_{args.seed}.pkl', 'rb'))
          exit_ = True
      except:
          exit_ = False
          pass
      ...
      SEED = 123 # seed for spliting dataset
      np.random.seed(SEED) 
      ...
      if args.dataset == 'photo' or args.dataset == 'computers':
          ...
          raise NotImplementedError('Dataset must be in "cora", "citeseer", "photo", "computers"')
      ...
      SEED = args.seed # seed for training
      np.random.seed(SEED) 
      torch.manual_seed(SEED) 
      torch.cuda.manual_seed(SEED)
      ...
      if args.attack == 'PGDCost':
          ...
          raise NotImplementedError('For Meta Attack, please use meta.py')
      else:
          raise NotImplementedError('Must be in "PGDCost", "Metattack"')
      ...
      if args.attack == 'Metattack':
        raise NotImplementedError('For Meta Attack, please use GCN_baseline_meta_cost.py')
        ...
        if not os.path.exists(f'../GCN_ADV_Train/node_costs/node_costs_{args.dataset}_{args.perturb_ratio}_cc{args.cost_constraint}_{args.seed}_{args.hyper_c_ratio}.pkl'):
            raise FileNotFoundError(f'../GCN_ADV_Train/node_costs/node_costs_{args.dataset}_{args.perturb_ratio}_cc{args.cost_constraint}_{args.seed}_{args.hyper_c_ratio}.pkl not found.'+' Please run ../GCN_ADV_Train/adv_train_pgd_cost_constraint.py with corrsponding args first.') 
        ...
        if args.cost_scheme == 'raw':
            ...
            raise NotImplementedError('cost_scheme not implemented, please check your spelling.')
        
        node_costs = node_costs.astype(np.float64)    
        print(f'new mean cost per node: {node_costs.mean()}')
        
        hyper_c = 0.001*253.45/perturbations * args.hyper_c_ratio
        ...
        raise NotImplementedError('Please specify "PGDCost" in args.attack')
        
      result = dict()
      print(args.cost_scheme, args.dataset)
      print('=== testing GCN on clean graph ===')
      if args.model=='MedianGCN':
        clean_acc, clean_auc = test_median(torch.tensor(adj), attr, model, None)  
      else:
      ...
      if not os.path.exists('results/'):
        os.makedirs('results/')
      pkl.dump(result, open(f'results/result_{args.model}_{args.attack}_{args.dataset}_{args.cost_scheme}_{args.perturb_ratio}_{args.cost_constraint}_{args.seed}_{args.hyper_c_ratio}.pkl', 'wb'))