...
flags.DEFINE_float('constraint_percentage', 0.05, 'perturb ratio of total edges.')#0.05
...
# theoretical_max_cost = n_node * (n_node-1)
eps_cost = 100 * FLAGS.constraint_percentage

# hyper_c = 0.01 / theoretical_max_cost * 1e6
hyper_c = 10
...
train_iter = 100 
# train_iter = 100
attack = PGDAttack(sess, model, features, None, eps_cost, FLAGS.att_steps, mu, adj, None)
...
for n in range(12):#FLAGS.train_steps# photo 8 computer 6 15!
    ...
    attack_feed_dict.update({placeholders['s'][i]: s for i, s in enumerate(zero_s)})
    attack_feed_dict.update({placeholders['g']: hyper_c})
    new_adv_support, now_s, now_loss1,edge_cost,attack_loss = attack.perturb(attack_feed_dict, FLAGS.discrete, attack_label, attack_label_mask, FLAGS.att_steps, node_cost = now_node_cost, eps_cost = eps_cost, ori_support=support) # PGD attack under discrete case
...
# pkl.dump(now_s, open(f'now_s_saves_new/{FLAGS.dataset}_{FLAGS.constraint_percentage}_{FLAGS.seed}_{hyper_c}.pkl', 'wb'))
# # node cost for graphlet and baseline
# pkl.dump(now_node_cost, open(f'node_costs_new/node_costs_{FLAGS.dataset}_{FLAGS.constraint_percentage}_{FLAGS.seed}_{hyper_c}.pkl','wb'))
# # save results
# pkl.dump(atk_accs, open(f'results_new/atk_accs_{FLAGS.dataset}_{FLAGS.constraint_percentage}_{FLAGS.seed}_{hyper_c}.pkl', 'wb'))
pkl.dump(atk_accs, open(f'results_new/atk_accs_{FLAGS.dataset}_{FLAGS.constraint_percentage}_{FLAGS.seed}_{hyper_c}_{FLAGS.hidden1}.pkl', 'wb'))
# pkl.dump(atk_aucs, open(f'results_new/atk_aucs_{FLAGS.dataset}_{FLAGS.constraint_percentage}_{FLAGS.seed}_{hyper_c}.pkl', 'wb'))
###############################################################################################


# ###############################################################################################
# # # recording now_s for poisoning 
# pkl.dump(now_s, open(f'now_s_saves_new/{FLAGS.dataset}_{FLAGS.constraint_percentage}_{hyper_c}.pkl', 'wb'))
# # # node cost for graphlet and baseline
# pkl.dump(now_node_cost, open(f'node_costs_new/node_costs_{FLAGS.dataset}_{FLAGS.constraint_percentage}_{hyper_c}.pkl','wb'))
# # # save results
# pkl.dump(atk_accs, open(f'results_new/atk_accs_{FLAGS.dataset}_{FLAGS.constraint_percentage}_{hyper_c}.pkl', 'wb'))
# pkl.dump(atk_aucs, open(f'results_new/atk_aucs_{FLAGS.dataset}_{FLAGS.constraint_percentage}_{hyper_c}.pkl', 'wb'))
# ###############################################################################################
