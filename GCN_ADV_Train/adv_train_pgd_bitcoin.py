...
for n in range(9):#FLAGS.train_steps# photo 8 computer 6 15!
    ...
    if n==0:
        ...
        for i in range(400): # 200
            ...
            if i % 10 == 0:
                cost, acc_clean, duration = evaluate(features, support, y_test, test_mask, placeholders,hyper_c,lmd,adj)      # evaluate model trained on clean graph
        ...
        cost, acc_clean, duration = evaluate(features, support, y_test, test_mask, placeholders,hyper_c,lmd,adj)      # evaluate model trained on clean graph
        atk_accs.append(acc_clean)
        print(f'Testing on clean graph: acc: {acc_clean:.4f}')
    ...
    attack_feed_dict.update({placeholders['s'][i]: s for i, s in enumerate(zero_s)})
    attack_feed_dict.update({placeholders['g']: hyper_c})
    new_adv_support, now_s, now_loss1,edge_cost,attack_loss= attack.perturb(attack_feed_dict, FLAGS.discrete, attack_label, attack_label_mask, FLAGS.att_steps, ori_support=support) # PGD attack under discrete case

    cost, atk_acc, duration = evaluate_attacked(features, new_adv_support, y_test, test_mask, placeholders, hyper_c, lmd, adj, now_s)
    print(f'Testing accuracy under attack of epoch {n+1}: {atk_acc:.4f}')
    atk_accs.append(atk_acc)
    # dump the generated attack for poisoning evaluation
    pkl.dump(now_s, open(f'now_s_saves/{FLAGS.dataset}_{FLAGS.perturb_ratio}.pkl', 'wb'))
    
    # new_adv_support is attack support
    # cost, acc_atk_last, duration, atk_last_pred = evaluate_attacked(features, support, y_test, test_mask, placeholders,hyper_c,lmd,adj, now_s)      # testing effect of attack without cost protection
    # print(f'Testing the effect attack at epoch {n+1}: acc: {acc_atk_last:.3f}')
    # atk_accs.append(acc_atk_last)

# save and dump node cost, uncomment below
# model.save(sess, save_name + '/' + save_name + '.h5')
pkl.dump(now_node_cost, open(f'./bitcoin_data/{FLAGS.dataset}_{FLAGS.perturb_ratio}_node_cost.pkl','wb'))
# pkl.dump(atk_accs, open(f'results/atk_accs_{FLAGS.dataset}_{FLAGS.perturb_ratio}.pkl', 'wb'))
