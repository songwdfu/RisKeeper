...
class PGDAttack:
  ...
  def perturb(self, feed_dict, discrete, y_test, test_mask, k, eps=None, ori_support=None):
    ...
    for epoch in range(k):
        ...
        # s \in [0,1]
        a,support,now_loss1,edge_cost,attack_loss = self.sess.run([self.model.a,self.model.placeholders['support'],self.model.loss2_no_train,self.model.now_edge_cost_no_train,self.model.attack_loss], feed_dict=feed_dict)
        print("total",now_loss1,"cost",edge_cost,"margin",attack_loss)
        upper_S_update, _ = bisection(a,self.eps,self.xi)                                                # find the s that solves optimal constraint (lag)
    ...
    return support_d if discrete else support,upper_S_update,now_loss1,edge_cost,attack_loss
